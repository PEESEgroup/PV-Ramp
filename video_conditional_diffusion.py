...
from utilities import load_split_manifest
...
class Unet3D(nn.Module):
    def __init__(
        ...
        *,
        ...
        if architecture not in {"paper", "legacy"}:
            raise ValueError("architecture must be 'paper' or 'legacy'.")
        ...
        if cross_attn is None:
            cross_mask = ([True] * num_resolutions if architecture == "paper"
        ...
        self.mid_cross_attn = LinearCrossAttention3D(mid_dim, mid_dim) if cross_mask[-1] else NullCrossAttn()
        ...
        for i in range(num_resolutions):
            ...
            if use_cross:
                ...
                self.cross_attn_down.append(NullCrossAttn())
    ...
    def _prepare_cond_features(self, cond_input):
        ...
        if T_cond != 20:
            raise ValueError("cond_input must contain exactly 4 past and 16 coarse future frames.")
        T_past = 4
    ...
    def forward(self, x, cond_input, time, x_self_cond=None):
        ...
        for i, (block1, block2, attn, downsample) in enumerate(self.downs):
            ...
            cur = cur + self.cross_attn_down[i](cur, cond_i)
            hs.append(cur)
            cur = downsample(cur)
        ...
        cur = cur + self.mid_cross_attn(cur, cond_mid)
        ...
        for i, (block1, block2, attn, upsample) in enumerate(self.ups):
            ...
            if self.architecture == "legacy":
                # Preserve the original double-residual behavior only for old
                # checkpoints. The paper architecture uses pure increments.
                if not isinstance(self.cross_attn_up_tex[i], NullCrossAttn):
                    tex = tex + cur
                if not isinstance(self.cross_attn_up_str[i], NullCrossAttn):
                    strc = strc + cur
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Video Conditional Diffusion")
    ...
    parser.add_argument("--split_manifest", default="data/split_manifest.json")
    return parser.parse_args(argv)


def build_diffusion(architecture="paper"):
    if architecture == "legacy":
        warnings.warn(
            "legacy diffusion attention does not match the paper architecture",
            RuntimeWarning,
        )
...
def main(argv=None):
    ...
    diffusion = build_diffusion(args.architecture)
    _, split_hash = load_split_manifest(args.split_manifest)
    trainer = Trainer(
        diffusion,
    ...
    if args.load_checkpoint:
        trainer.load_checkpoint(args.load_checkpoint, architecture=args.architecture)
    trainer.train()
