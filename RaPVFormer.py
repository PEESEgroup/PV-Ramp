...
@torch.no_grad()
def evaluate_epoch(model: nn.Module, loader, device, criterion, mse_loss):
    model.eval()
    tot_loss, tot_mse = 0.0, 0.0
    for _, loaddata in enumerate(loader, 0):
        # Assumes: (idx, past_imgs, future_imgs, past_pv, target_pv)
        loaddata = [data.to(device) for data in loaddata]
        _, input_frames, target_frames, input_pv, target_pv = loaddata
        pred_pv, ramp_logits = model(
            past_imgs=input_frames, past_pv=input_pv.squeeze(-1), future_imgs=target_frames
        )
...
def main():
    ...
    # ---- Dataset: HybridVideoDataset assumed to provide all ground-truths
    dataset = HybridVideoDataset(stride=args.stride)
    ...
    train_dataset, val_dataset, test_dataset = split_hybrid_dataset(dataset, [n_train, n_val, n_test], generator=g)
