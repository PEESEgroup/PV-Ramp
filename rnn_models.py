...
class PhyCell_Cell(nn.Module):
    ...
    def forward(self, x, hidden): # x [batch_size, hidden_dim, height, width]      
        ...
        # A correction gate is a convex interpolation coefficient and must stay
        # in [0, 1].  The manuscript's tanh in Eq. (7) is therefore a typo.
...
class PhyCell(nn.Module):
    ...
    def forward(self, input_, first_timestep=False): # input_ [batch_size, 1, channels, width, height]    
        ...
        if (first_timestep):   
            self.initHidden(batch_size, input_.device, input_.dtype) # init Hidden at each forward start
    ...
    def initHidden(self, batch_size, device=None, dtype=None):
        self.H = [] 
...
class ConvLSTM(nn.Module):
    ...
    def initHidden(self, batch_size, device=None, dtype=None):
        ...
        dtype = torch.float32 if dtype is None else dtype
        for i in range(self.n_layers):
            shape = (batch_size, self.hidden_dims[i], self.input_shape[0], self.input_shape[1])
            self.H.append(torch.zeros(shape, device=device, dtype=dtype))
            self.C.append(torch.zeros(shape, device=device, dtype=dtype))
...
class PhyDNet(nn.Module):
    ...
    @staticmethod
    def _validate_sequence_inputs(past_rgb, past_sun, future_sun):
        if past_rgb.ndim != 5 or past_sun.ndim != 5 or future_sun.ndim != 5:
            raise ValueError("Sequence inputs must have shape [B,T,C,H,W].")
        if past_rgb.shape[1:3] != (16, 3):
            raise ValueError("past_rgb must contain 16 RGB frames.")
        if past_sun.shape[1:3] != (16, 1) or future_sun.shape[1:3] != (16, 1):
            raise ValueError("past_sun and future_sun must each contain 16 one-channel masks.")
        expected = (past_rgb.shape[0], past_rgb.shape[-2], past_rgb.shape[-1])
        for name, tensor in (("past_sun", past_sun), ("future_sun", future_sun)):
            actual = (tensor.shape[0], tensor.shape[-2], tensor.shape[-1])
            if actual != expected:
                raise ValueError(f"{name} batch/spatial dimensions do not match past_rgb.")

    def predict_sequence(self, past_rgb, past_sun, future_sun):
        """Forecast 16 RGB frames without accepting any future observed RGB."""
        self._validate_sequence_inputs(past_rgb, past_sun, future_sun)
        for index in range(past_rgb.shape[1] - 1):
            frame = torch.cat([past_rgb[:, index], past_sun[:, index]], dim=1)
            self(frame, first_timestep=index == 0)

        decoder_input = torch.cat([past_rgb[:, -1], past_sun[:, -1]], dim=1)
        outputs = []
        for index in range(future_sun.shape[1]):
            prediction = self(decoder_input)
            outputs.append(prediction)
            decoder_input = torch.cat([prediction, future_sun[:, index]], dim=1)
...
class FrameLoss(nn.Module):
    ...
    def forward(self, img1: torch.Tensor, img2: torch.Tensor) -> torch.Tensor:
        total, _, _ = self.components(img1, img2)
        return total

    def components(self, img1: torch.Tensor, img2: torch.Tensor):
        ssim_val = self.ssim_loss(img1, img2)
        l1_val = self.l1_loss(img1, img2)
        total = self.alpha * ssim_val + (1 - self.alpha) * l1_val
        return total, ssim_val, l1_val
