...
class SkyVideoDataset(Dataset):
    def __init__(self, stride = 1, hdf5_path = None):
        ...
        sky_image = torch.from_numpy(sky_image / 255.0).contiguous().float()  # normalize to [0, 1]
        ...
        logging.info(f"Dataset size: {self.length}")

    def __getitem__(self, idx):
        stidx = self.start_idx[idx]

        image_data = self.sky_image[stidx : stidx+31 : self.stride]
        input_len = (image_data.shape[0])//2
        input_frames = image_data[: input_len]
        target_frames = image_data[input_len:]

        pv_data = self.pv_output[stidx : stidx+31 : self.stride]
        input_pv = pv_data[:input_len]
        target_pv = pv_data[input_len:]

        out = [stidx, input_frames, target_frames, input_pv, target_pv]
        return out
